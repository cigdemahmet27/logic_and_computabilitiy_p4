        ...
        with open(config.BCP_OUTPUT_FILE, "r") as f:
            ...
            for line in lines:
                ...
                if parsing_vars and line:
                    # Handle format: "1    | UNASSIGNED" or "1    | TRUE"
                    if "|" in line:
                        parts = line.split("|")
                    ...
                    # Also handle old format: "1" then next line "UNASSIGNED"
                    elif line.isdigit():
                        current_var = int(line)
                    elif current_var is not None:
                        # This line must be the state (TRUE, FALSE, UNASSIGNED)
                        if line == "UNASSIGNED":
                            result["unassigned_vars"].append(current_var)
                        elif line == "TRUE":
                            result["assignments"][current_var] = True
                        elif line == "FALSE":
                            result["assignments"][current_var] = False
                        current_var = None # Reset for next var
